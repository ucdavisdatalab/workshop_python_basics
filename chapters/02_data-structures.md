...
* Describe and differentiate lists, tuples, dictionaries, and series
* Describe and differentiate `None` (or `null`) and `NaN`
* Locate missing values in a data frame (or series)
* Select columns of a data frame
* Filter rows of a data frame (or elements of a series) on a condition
...
Python and Polars. Then, it explains how to use this knowledge during data
analysis.
...
FIXME:
We will be working with two packages in this chapter, NumPy and Polars. Start
...
{numref}`getting-started` described how to use Polars to read a tabular dataset
into Python. As an example, you saw how to load the
...
A **series** is a one-dimensional collection of values. Series are a
fundamental data structure in Polars, since each column in a data frame is a
series.

For example, in the California least tern data from {ref}`sec-hello-data`, the
`site_name` column is a series. Take a look at the first few elements with the
`.head` method:
...
Like all series, this series is **ordered**, which just means the values, or
**elements**, have specific positions. The value of the 1st element is
`PITTSBURG POWER PLANT`, the 2nd is `ALBANY CENTRAL AVE`, the 10th is `NBVC
POINT MUGU`, and so on.

Notice that the elements of this series are all strings. The elements of a
series must all be the same type of data, so we say series are **homogeneous**.
A series can contain strings, integers, decimal numbers, or any of several
other types of data, but not a mix of these all at once.
...
test out some code. You can do so with the `pl.Series` function.

One way to pass a sequence of elements to the `pl.Series` function is in a
**list**. You can create a list by enclosing any number of comma-separated
elements in square brackets `[]`, like this:

```{code-cell}
[10, 20, 30]
```

For now, think of lists as Python's built-in equivalent to (Polars') series.
We'll explain the ways lists differ from series in {ref}`sec-lists`.

With that in mind, here's how to create two different series. The first
contains integers, whereas the second contains strings:
...
If the elements of the list you pass to the `pl.Series` function have different
data types, the function will raise an error:
...
Polars will print the name when you print the series, and will automatically
use the name if you later add the series to a data frame. You can get or set
the name on a series through the `.name` attribute:
...
You can use indexing to get an element of a series based on the element's
...
you want to get. For data frames, you can use a position or a column name as
the index. For a series, you can only use a position.

Consider this series of strings:

```{code-cell}
s = pl.Series("strings", ["hi", "hello", "4.2"])
s
```
...
You can set the element of a series by assigning a value at that index. So the
code to change the first element of `s` to the string "yay" is:
...
characteristics. For instance, data scientists tend to think about whether data
...
`int`     | `-8`, `0`, `42` | Integer numbers
`float`   | `-2.1`, `0.5`   | Decimal numbers
...
function (we used this function to check the type of a data frame in
{numref}`inspecting-dataframe`). Take a look at the types of a few other
objects:
...
Python can even parse strings into numbers:
...
Polars provides its own set of data types, with a few important
differences:

1. Each numeric type specifies a **bit size**, the number of bits of memory
   used to store a value of that type.
2. 

its own set of data types that mostly parallel Python's. Some
of these are listed in the following table; the Polars documentation has [the
complete list][polars-types].

[polars-types]: https://docs.pola.rs/user-guide/concepts/data-types-and-structures/#appendix-full-data-types-table

At the expense of being more complicated, the Pandas/NumPy types tend to be
more specific and consistent. They provide programmers with greater control
over how data are stored in memory, which makes it possible to write more
efficient code. For computations that generate or process a large amount of
data, as is often the case in research computing, efficiency is a major
concern.

The names of the Pandas/NumPy types in the table all end in numbers such as
`32` or `64`. These numbers indicate the **bit size**, the number of bits of
memory used to store a value of that type.
...
You can also access the element types for a data frame with the `.dtypes`
attribute:
...
For a series or array, you can instead use `.dtype` to get the element type:
...
Data frames, introduced in the previous chapter, are an example of a
...
Type      | Example           | Description
--------- | ----------------- | -----------
`list`    | `[True, 1, "hi"]` | Ordered collection of heterogeneous elements
`tuple`   | `(True, 1, "hi")` | Ordered collection of heterogeneous elements
`dict`    | `{"hi": -3.5}`    | Dictionary (key-value pairs)


(sec-lists)=
### Lists

A **list** is a general-purpose one-dimensional container. Lists are built into
Python; you don't even need to load a module in order to use them. You can
create a list by enclosing comma-separated values in square brackets `[]`, like
this:
...
Lists are **ordered**, which just means the values, or **elements**, have
specific positions. The 1st element is `10`, the 2nd is `20`, the 5th is `50`,
and so on.

The elements of a list can have different types (we say the elements are
**heterogeneous**). There is no expectation that they will be same. For
instance, this list contains a number, string, and another list (with one
element):
